...
#ifdef Q_OS_WIN
    const QString AdapterGatewayInfo::wintunAdapterName = "Windscribe Windtun420";
    const QString AdapterGatewayInfo::tapAdapterName = "Windscribe VPN";
#endif
...
{
    return QString("adapter name = %1; adapter IP = %2; gateway IP = %3; remote IP = %4; dns = (%5)")
            .arg(adapterName_).arg(adapterIp_).arg(gateway_).arg(remoteIp_).arg(dnsServers_.join(','));
}
