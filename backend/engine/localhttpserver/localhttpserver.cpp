...
void LocalHttpServer::startServer()
{
    ...
    QString appPath = QCoreApplication::applicationDirPath() + "/../Resources";
    QString exePath = appPath + "/wsappcontrol";
#endif

    if (!ExecutableSignature::verify(exePath))
    {
        qCDebug(LOG_BASIC) << "wsappcontrol.exe signature incorrect";
        return;
    }
