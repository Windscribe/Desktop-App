...
// Supporting 32-Bit user-mode applications (added by Alexey Azanov)
typedef struct _TUN_REGISTER_RINGS_32
{
    struct
    {
        ...
        /* Pointer to client allocated ring */
        VOID *POINTER_32 Ring;

        /* On send: An event created by the client the Wintun signals after it moves the Tail member of the send ring.
         * On receive: An event created by the client the client will signal when it moves the Tail member of
         * the receive ring if receive ring is alertable. */
        VOID *POINTER_32 TailMoved;
    } Send, Receive;
...
TunRegisterBuffers(_Inout_ TUN_CTX *Ctx, _Inout_ IRP *Irp)
{
    ...
    // added by Alexey Azanov (supporting 32-Bit)
    TUN_REGISTER_RINGS Rrb;

    // 64-bit struct size
    if (Stack->Parameters.DeviceIoControl.InputBufferLength == sizeof(TUN_REGISTER_RINGS))
    {
        RtlCopyMemory(&Rrb, Irp->AssociatedIrp.SystemBuffer, sizeof(TUN_REGISTER_RINGS));
    }
    // 32-bit struct size
    else if (Stack->Parameters.DeviceIoControl.InputBufferLength == sizeof(TUN_REGISTER_RINGS_32))
    {
        TUN_REGISTER_RINGS_32 *Rrb32 = Irp->AssociatedIrp.SystemBuffer;
        Rrb.Receive.RingSize = Rrb32->Receive.RingSize;
        Rrb.Receive.Ring = Rrb32->Receive.Ring;
        Rrb.Receive.TailMoved = Rrb32->Receive.TailMoved;

        Rrb.Send.RingSize = Rrb32->Send.RingSize;
        Rrb.Send.Ring = Rrb32->Send.Ring;
        Rrb.Send.TailMoved = Rrb32->Send.TailMoved;
    }
    else
    {
