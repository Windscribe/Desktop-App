...
func (device *Device) IpcSetOperation(socket *bufio.Reader) *IPCError {
	...
	for scanner.Scan() {
		...
		if deviceConfig {
			...
			case "bind_socket_if4":

				values := strings.Split(value , ",")
				ifindex, err := strconv.ParseUint(values[0], 10, 32)
				if err != nil {
					logError.Println("Failed to parse bind_socket_if4:", err)
					return &IPCError{ipc.IpcErrorInvalid}
				}
				blackhole, err := strconv.ParseBool(values[1])
				if err != nil {
					logError.Println("Failed to parse bind_socket_if4:", err)
					return &IPCError{ipc.IpcErrorInvalid}
				}
			...
			case "bind_socket_if6":

				values := strings.Split(value , ",")
				ifindex, err := strconv.ParseUint(values[0], 10, 32)
				if err != nil {
					logError.Println("Failed to parse bind_socket_if6:", err)
					return &IPCError{ipc.IpcErrorInvalid}
				}
				blackhole, err := strconv.ParseBool(values[1])
				if err != nil {
