...
// Windscribe modification: Wintun -> Windtun420
func initializeNamespace() error {
	...
	boundary, err := namespaceapi.CreateBoundaryDescriptor("Windtun420")
	if err != nil {
	...
	for {
		_, err = namespaceapi.CreatePrivateNamespace(wintunObjectSecurityAttributes, boundary, "Windtun420")
		if err == windows.ERROR_ALREADY_EXISTS {
			_, err = namespaceapi.OpenPrivateNamespace(boundary, "Windtun420")
...
func (pool Pool) takeNameMutex() (windows.Handle, error) {
	...
	// Windscribe modification: Wintun -> Windtun420
	mutexName := `Windtun420\Windtun420-Name-Mutex-` + hex.EncodeToString(b2.Sum(nil))
