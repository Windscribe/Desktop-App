...
UninstallInfo::UninstallInfo(const std::wstring &installPath, double weight) : IInstallBlock(weight, L"uninstall_info")
{
	installPath_ = installPath;
	uninstallExeFilename_ = Path::AddBackslash(installPath_) + ApplicationInfo::instance().getUninstall();
}
...
void UninstallInfo::registerUninstallInfo(const wstring &uninstallRegKeyBaseName)
{
	...
	setStringValue(hUninstallKey, L"InstallLocation", Path::AddBackslash(installPath_));
	setStringValue(hUninstallKey, NEWREGSTR_VAL_UNINSTALLER_DISPLAYNAME.c_str(), ApplicationInfo::instance().getName());
    setStringValue(hUninstallKey, L"DisplayIcon", Path::AddBackslash(installPath_) + ApplicationInfo::instance().getName() + L".exe");
	...
	uint64_t estimatedSize = calculateDirSize(installPath_, NULL);
