    ...
    for (auto it = pingInfos_.begin(); it != pingInfos_.end(); ++it)
    {
        for (auto remoteIt = it->remotes.begin(); remoteIt != it->remotes.end(); ++remoteIt)
        {
            if (remoteIt->isHostname)
            {
                ...
                {
                    strListIps << ipIt->ip;
                    allIps << PingIpInfo(ipIt->ip, QString(), it->customConfig->name(), it->customConfig->nick(), PingHost::PING_ICMP);
                }
            }
            else
            {
                strListIps << remoteIt->ipOrHostname.ip;
                allIps << PingIpInfo(remoteIt->ipOrHostname.ip, QString(), it->customConfig->name(), it->customConfig->nick(), PingHost::PING_ICMP);
            }
        }
    }
