...
PreferenceGroup::PreferenceGroup(ScalableGraphicsObject *parent, const QString &desc, const QString &descUrl)
    : BaseItem(parent, 0), desc_(desc), errorDesc_(""), descUrl_(descUrl), descRightMargin_(PREFERENCES_MARGIN), descHeight_(0),
...
void PreferenceGroup::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    // description/link rounded rectangle
    if (!descHidden_ && (size() != 0 || !desc_.isEmpty() || (error_ && !errorDesc_.isEmpty()) || !drawBackground_))
    {
        ...
        painter->fillPath(stroker.createStroke(path), QBrush(Qt::white));
    ...
    if (size() != 0 && drawBackground_)
    {
        ...
        path.addRoundedRect(boundingRect().adjusted(borderWidth_*G_SCALE,
                                                    ...
                                                    -borderWidth_*G_SCALE,
                                                    -borderWidth_*G_SCALE - descHeight_),
    ...
    int descVMargin = DESCRIPTION_MARGIN*G_SCALE;
    if (size() == 0)
    {
        // use full margin if there are no items in this group
        descVMargin = PREFERENCES_MARGIN*G_SCALE;
    }
    ...
    if (error_)
    {
        ...
        errorIcon_->draw(PREFERENCES_MARGIN*G_SCALE,
                         boundingRect().height() - descHeight_/2 - ICON_HEIGHT/2*G_SCALE,
                         ICON_WIDTH*G_SCALE,
                         ICON_HEIGHT*G_SCALE,
                         painter);
...
void PreferenceGroup::updatePositions()
{
    ...
    {
        descRightMargin_ = (2*PREFERENCES_MARGIN + ICON_WIDTH)*G_SCALE;
    }
    else
    {
        descRightMargin_ = PREFERENCES_MARGIN*G_SCALE;
    }
    ...
    {
        int descVMargin = (2*DESCRIPTION_MARGIN + borderWidth_)*G_SCALE;
        if (size() == 0)
        {
            // use full margin if there are no items in this group
            descVMargin = (2*PREFERENCES_MARGIN + borderWidth_)*G_SCALE;
        }
    ...
    if (!error_ && !descUrl_.isEmpty())
    {
        ...
        icon_->setPos(boundingRect().width() + PREFERENCES_MARGIN*G_SCALE - descRightMargin_,
                      boundingRect().height() - descHeight_/2 - ICON_HEIGHT/2*G_SCALE);
    }
    else
    {
