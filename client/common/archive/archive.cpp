...
#ifndef _WIN32
bool Archive::isCorrect() const
{
    return pData != NULL && file_size != 0;
}
#endif
...
WRes Archive::MyCreateDir(const UInt16 *name)
{
    ...
    SRes __result__ = Utf16_To_Char(&buf, name);
    if (__result__ != 0)
    {
...
SRes Archive::extractionFile(const UInt32 &i)
{
    ...
    if (extract)
    {
        ...
        if (isSymLink)
        {
            ...
            char *symlinkStr = new char[processedSize];
...
//Set file permission(for OS X only)
#ifndef _WIN32
    {
        ...
        SRes __result__ = Utf16_To_Char(&buf, destPath);
        if (__result__ != 0)
        {
            return __result__;
        }

        if (chmod(reinterpret_cast<const char *>(buf.data), perm) != 0)
        {
            return SZ_ERROR_FAIL;
        }
