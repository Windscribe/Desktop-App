...
- (void)sendPingWithData:(NSData *)data {
    ...
    if ( (bytesSent > 0) && (((NSUInteger) bytesSent) == packet.length) ) {
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didFailToSendPacket:sequenceNumber:error:)] ) {
            [strongDelegate simplePing:self didFailToSendPacket:packet sequenceNumber:self.nextSequenceNumber error:error]; // NOLINT(clang-analyzer-nullability.NullPassedToNonnull)
        }
    }
...
- (void)startWithHostAddress {
    ...
    if (err != 0) {
        ...
        if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didStartWithAddress:)] ) {
            [strongDelegate simplePing:self didStartWithAddress:self.hostAddress]; // NOLINT(clang-analyzer-nullability.NullablePassedToNonnull)
        }
    }
    assert(fd == -1);
}
...
- (void)hostResolutionDone {
    ...
    addresses = (__bridge NSArray *) CFHostGetAddressing(self.host, &resolved); // NOLINT(clang-analyzer-nullability.NullablePassedToNonnull)
...
- (void)start {
    ...
    CFHostSetClient(self.host, HostResolveCallback, &context); // NOLINT(clang-analyzer-nullability.NullablePassedToNonnull)
