...
DecoyTraffic::DecoyTraffic(boost::asio::io_context &io_context, WSNetHttpNetworkManager *httpNetworkManager) :
    uploadTraffic_(io_context, httpNetworkManager, true),
    downloadTraffic_(io_context, httpNetworkManager, false)
{
}
...
void DecoyTraffic::setFakeTrafficVolume(std::uint32_t volume)
{
    ...
    uploadTraffic_.setFakeTrafficVolume(volume);
    downloadTraffic_.setFakeTrafficVolume(volume);
