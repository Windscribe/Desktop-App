...
void TooltipBasic::paintEvent(QPaintEvent */*event*/)
{
    ...
    painter.drawText(marginWidth_*G_SCALE + additionalTailWidth(), 20*G_SCALE, text_);
...
void TooltipBasic::recalcWidth()
{
    ...
    int otherWidth = marginWidth_* 2 * G_SCALE; // margins + spacing
...
{
    QFontMetrics fm(font_);
    height_ = (marginHeight_* 2 * G_SCALE) + fm.height() + additionalTailHeight();
}
