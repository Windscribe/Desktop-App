...
Logger::Logger(const wchar_t *installDir)
{
    ...
    logFile.append(L"log" WS_PRODUCT_NAME_LOWER_W L"installhelper.txt");

    errno_t result = _wfopen_s(&file_, logFile.native().c_str(), L"wt,ccs=UTF-8");
    if ((result != 0) || (file_ == nullptr)) {
        WSDebugMessage(WS_PRODUCT_NAME_W L" install helper failed to open its log file '%s' (%d)", logFile.native().c_str(), result);
    }
}
