...
-(void) execution
{
    ...
    if (self.factoryReset) {
        ...
        // Use `defaults delete` to clear preferences through cfprefsd rather than
        // deleting plist files directly.  Removing the files with `rm` and then
        // killing cfprefsd is racy: cfprefsd may flush its in-memory cache back
        // to disk during SIGTERM shutdown, recreating the files we just deleted.
        // `defaults delete` removes the domain from cfprefsd's cache and the file
        // atomically, avoiding the race.
        [self runProcess:@"/usr/bin/defaults" args:@[@"delete", @"com.windscribe.Windscribe"]];
        [self runProcess:@"/usr/bin/defaults" args:@[@"delete", @"com.windscribe.Windscribe2"]];
        [self runProcess:@"/usr/bin/defaults" args:@[@"delete", @"com.windscribe.client"]];
