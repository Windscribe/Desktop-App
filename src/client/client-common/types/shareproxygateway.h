...
struct ShareProxyGateway
{
    ...
    ShareProxyGateway(const QJsonObject &json)
    {
        ...
        if (json.contains(kJsonProxySharingModeProp) && json[kJsonProxySharingModeProp].isDouble()) {
            proxySharingMode = enumFromInt<PROXY_SHARING_TYPE>(json[kJsonProxySharingModeProp].toInt());
        }
    ...
    void fromIni(const QSettings &settings)
    {
        ...
        proxySharingMode = enumFromString<PROXY_SHARING_TYPE>(settings.value(kIniProxySharingModeProp, enumToString(proxySharingMode)).toString());
    ...
    void toIni(QSettings &settings) const
    {
        ...
        settings.setValue(kIniProxySharingModeProp, enumToString(proxySharingMode));
    ...
    QJsonObject toJson(bool isForDebugLog) const
    {
        ...
        if (isForDebugLog) {
            json["proxySharingModeDesc"] = enumToString(proxySharingMode);
    ...
    void validate()
    {
        proxySharingMode = enumFromInt<PROXY_SHARING_TYPE>(static_cast<int>(proxySharingMode));
        if (port >= 65536) {
