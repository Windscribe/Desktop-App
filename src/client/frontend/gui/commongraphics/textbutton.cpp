...
TextButton::TextButton(QString text, const FontDescr &fd, QColor color, bool bSetClickable, ScalableGraphicsObject *parent, int addWidth, bool bDrawWithShadow) : ClickableGraphicsObject(parent),
    ...
    textAlignment_(Qt::AlignLeft | Qt::AlignVCenter), textUnderline_(false), textBigUnderline_(false), unhoverOnClick_(true)
{
    if (bDrawWithShadow) {
...
void TextButton::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (textShadow_) {
        ...
        painter->drawText(boundingRect().adjusted(0, 0, 0, textBigUnderline_ ? -4*G_SCALE : 0), textAlignment_ , elidedText);
    }
    // Draw an underline below the text
    if (textBigUnderline_) {
        painter->drawLine(1*G_SCALE, boundingRect().toRect().bottom() - 1*G_SCALE, boundingRect().toRect().width(), boundingRect().toRect().bottom() - 1*G_SCALE);
    }
}
...
void TextButton::setBigUnderline(bool on)
{
    textBigUnderline_ = on;
    recalcBoundingRect();
}
