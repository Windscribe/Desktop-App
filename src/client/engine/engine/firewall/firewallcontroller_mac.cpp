...
FirewallController_mac::FirewallController_mac(QObject *parent, Helper *helper) :
    FirewallController(parent), helper_(helper), isWindscribeFirewallEnabled_(false), isAllowLanTraffic_(false), connectingIp_("")
{
...
void FirewallController_mac::firewallOn(const QString &connectingIp, const QSet<QString> &ips, bool bAllowLanTraffic, bool isVpnConnected)
{
    ...
    if (!isWindscribeFirewallEnabled_) {
        ...
        QString pfConfig = generatePfConf(connectingIp, ips, bAllowLanTraffic, interfaceToSkip_);
        if (!pfConfig.isEmpty()) {
...
QString FirewallController_mac::generatePfConf(const QString &connectingIp, const QSet<QString> &ips, bool bAllowLanTraffic, const QString &interfaceToSkip)
{
    QString pf = "";
    ...
    vpnTrafficAnchor.addRules(vpnTrafficRules(connectingIp, interfaceToSkip));
...
QStringList FirewallController_mac::vpnTrafficRules(const QString &connectingIp, const QString &interfaceToSkip) const
{
    QStringList rules;
...
void FirewallController_mac::updateVpnAnchor() {
    Anchor vpnTrafficAnchor("windscribe_vpn_traffic");
    vpnTrafficAnchor.addRules(vpnTrafficRules(connectingIp_, interfaceToSkip_));
