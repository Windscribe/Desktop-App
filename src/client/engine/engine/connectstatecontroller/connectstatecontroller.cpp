...
void ConnectStateController::setDisconnectedState(DISCONNECT_REASON reason, ConnectError err)
{
    QMutexLocker locker(&mutex_);
...
ConnectError ConnectStateController::connectionError()
{
    QMutexLocker locker(&mutex_);
    return err_;
}
